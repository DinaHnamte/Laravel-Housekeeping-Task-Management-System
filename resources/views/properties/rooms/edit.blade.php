    ...
    <div class="mx-auto max-w-2xl">
        <div class="mb-8">
            <h1 class="bg-gradient-to-r from-slate-900 to-slate-600 bg-clip-text text-3xl font-bold text-transparent">Edit
                Room</h1>
            <p class="mt-2 text-sm text-slate-600">Update room information and settings</p>
        </div>

        <div class="rounded-2xl border border-slate-200/50 bg-white/80 p-8 shadow-xl shadow-slate-200/50 backdrop-blur-sm">
            <form action="{{ route("properties.rooms.update", [$property, $room]) }}" method="POST" class="space-y-6">
                @csrf
                @method("PUT")
                ...
                <div class="space-y-2">
                    ...
                    <div class="rounded-lg border border-slate-200 bg-slate-50 px-4 py-3 text-slate-600">
                        {{ $room->property->name ?? "No Property" }}
                    </div>
                ...
                <div class="space-y-2">
                    <div class="flex items-center space-x-3">
                        <input type="checkbox" id="is_default" name="is_default" value="1"
                            {{ old("is_default", $room->is_default) ? "checked" : "" }}
