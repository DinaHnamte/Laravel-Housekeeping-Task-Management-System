...
@section("sidebar-content")
                ...
                @foreach ($tasks as $task)
                    ...
                    <div class="task-card group relative overflow-hidden rounded-xl border border-slate-200/50 bg-white/80 p-6 shadow-lg shadow-slate-200/50 backdrop-blur-sm transition-all duration-300 hover:shadow-xl hover:shadow-slate-300/50"
                        ...
                        <div class="mb-4">
                            ...
                            <!-- Hidden file input -->
                            <input type="file" class="task-photo hidden" id="photo_{{ $task->id }}"
                                name="tasks[{{ $task->id }}][photo]" accept="image/*">

                            <!-- Camera and Gallery buttons -->
                            <div class="flex flex-col gap-2 sm:flex-row">
                                <button type="button" onclick="openCameraForTask('{{ $task->id }}')"
                                    class="inline-flex flex-1 items-center justify-center gap-2 rounded-lg border border-green-600 bg-green-50 px-3 py-2 text-sm font-semibold text-green-700 transition-all duration-200 hover:bg-green-100 active:scale-95">
                                ...
                                <button type="button" onclick="openGalleryForTask('{{ $task->id }}')"
                                    class="inline-flex flex-1 items-center justify-center gap-2 rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-semibold text-slate-700 transition-all duration-200 hover:bg-slate-50 active:scale-95">
                                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <!-- Preview area -->
                            <div id="preview_{{ $task->id }}" class="mt-3 hidden">
                                <img id="preview_img_{{ $task->id }}" src="" alt="Preview"
    ...
    <script>
        ...
        function openCameraForTask(taskId) {
            const input = document.getElementById('photo_' + taskId);
            // Create a temporary input with capture attribute to trigger camera
            const tempInput = document.createElement('input');
            tempInput.type = 'file';
            tempInput.accept = 'image/*';
            tempInput.capture = 'environment'; // Use rear camera
        ...
        function openGalleryForTask(taskId) {
            const input = document.getElementById('photo_' + taskId);
        ...
        function showPreview(taskId, file) {
            const previewDiv = document.getElementById('preview_' + taskId);
            const previewImg = document.getElementById('preview_img_' + taskId);
