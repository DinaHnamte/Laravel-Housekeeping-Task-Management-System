...
@section("sidebar-content")
    <div class="mx-auto max-w-2xl">
        <div class="mb-8">
            ...
            <p class="mt-2 text-sm text-slate-600">Update assignment information and task selection</p>
        </div>

        <div class="rounded-2xl border border-slate-200/50 bg-white/80 p-8 shadow-xl shadow-slate-200/50 backdrop-blur-sm">
            <form action="{{ route("checklists.update", $checklist) }}" method="POST" class="space-y-6">
                @csrf
                @method("PUT")
                    ...
                    <select id="property_id" name="property_id"
                        ...
                        @foreach ($properties as $property)
                            <option value="{{ $property->id }}"
                                {{ old("property_id", $checklist->property_id) == $property->id ? "selected" : "" }}>
                    ...
                    <select id="user_id" name="user_id"
                        ...
                        @foreach ($housekeepers as $housekeeper)
                            <option value="{{ $housekeeper->id }}"
                                {{ old("user_id", $checklist->user_id) == $housekeeper->id ? "selected" : "" }}>
                ...
                <div class="space-y-2">
                    ...
                    <input type="date" id="assignment_date" name="assignment_date"
                        value="{{ old("assignment_date", $checklist->assignment_date->format("Y-m-d")) }}"
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-2">
                        ...
                        <input type="time" id="start_time" name="start_time"
                            value="{{ old("start_time", $checklist->start_time ? $checklist->start_time->format("H:i") : "") }}"
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="time" id="end_time" name="end_time"
                            value="{{ old("end_time", $checklist->end_time ? $checklist->end_time->format("H:i") : "") }}"
                ...
                <div class="space-y-2">
                    ...
                    <select id="status" name="status"
                        class="@error("status") border-red-500 focus:border-red-500 focus:ring-red-500/20 @enderror w-full rounded-lg border border-slate-300 px-4 py-3 text-slate-900 transition-all duration-200 focus:border-slate-500 focus:ring-2 focus:ring-slate-500/20"
                        required>
                        <option value="pending" {{ old("status", $checklist->status) == "pending" ? "selected" : "" }}>
                            Pending</option>
                        <option value="in_progress"
                            {{ old("status", $checklist->status) == "in_progress" ? "selected" : "" }}>In Progress
                        </option>
                        <option value="completed"
                            {{ old("status", $checklist->status) == "completed" ? "selected" : "" }}>Completed</option>
                        <option value="cancelled"
                            {{ old("status", $checklist->status) == "cancelled" ? "selected" : "" }}>Cancelled</option>
                    </select>
                    @error("status")
                        <p class="text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <!-- Task Selection -->
                <div class="space-y-2" id="task-selection-section">
                    ...
                    <div id="tasks-container" class="max-h-64 space-y-2 overflow-y-auto">
                        @if ($checklist->property->rooms)
                            @foreach ($checklist->property->rooms as $room)
                                @if ($room->tasks && $room->tasks->count() > 0)
                                    <div class="mb-4 rounded-lg border border-slate-200 bg-slate-50 p-3">
                                        <div class="mb-2 font-semibold text-slate-700">{{ $room->name }}</div>
                                        @foreach ($room->tasks as $task)
                                            <label
                                                class="flex cursor-pointer items-center space-x-2 rounded px-2 py-1 hover:bg-slate-100">
                                                <input type="checkbox" name="task_ids[]" value="{{ $task->id }}"
                                                    class="rounded border-slate-300 text-slate-600 focus:ring-slate-500"
                                                    {{ in_array($task->id, $checklist->tasks->pluck("id")->toArray()) ? "checked" : "" }}>
                ...
                <div class="space-y-2">
                    ...
                    <textarea id="notes" name="notes" rows="3"
                        ...
                        placeholder="Add any special instructions or notes for this assignment">{{ old("notes", $checklist->notes) }}</textarea>
                    @error("notes")
                        <p class="text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
